...
{
  static const std::regex id_filter("-?(XL|SD|XF|QD|BN|DL|TS|DT|HP|GT|AZ|AZU|UT|BT|BC|SP|TIX|FG|BW|UW|UM|FD|GA|WW|ZO|PI|AT|TR[12]|qB[123]|DE[1])(\\S*)-?");
  static const std::regex ua_filter(R"((\d+.\d+.\d+.\d+))");
  static const std::regex consume_filter(R"(((dt|hp|xm)/torrent|Gopeed dev|Rain \d+(\.\d+)*|(Taipei-torrent( dev)?|cacao_torrent|[Ff]olx.*|aria2.*|.*sniffer.*|[Cc]oc.*|BitTorrent.*||[Uu]nknown.*)))", std::regex_constants::icase);

  QString country = Net::GeoIPManager::instance()->lookup(QHostAddress(info.ip.data()));
    if (country == QLatin1String("CN") || country == QLatin1String("HK")) {
      return true;
    }

    if (std::regex_match(info.pid.data(), info.pid.data() + 8, id_filter))
        return true;
    if (std::regex_match(info.client, ua_filter))
        return true;
